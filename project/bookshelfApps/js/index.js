...
class Bookshelf {
    ...
    addBukuToLocalStorage(className) {
        // const json = JSON.parse(className.getBookshelf());
        localStorage.setItem(BOOK, className.getBookshelf());
    }

    getBukuList() {
        let arr = [];
        if (localStorage.getItem(BOOK) !== null) arr = localStorage.getItem(BOOK).split(',');
        // console.log(arr); ;
        // console.log(arr);
...
const bookStorage = new Bookshelf();
...
function clickMiddleToSet () {
    for (let i = 0; i < document.querySelectorAll('.box-books').length; i++) {
        document.querySelectorAll('.box-books')[i].addEventListener('click', () => {
            console.log('gacorr' + i);
            appearRightToSet();
            // document.querySelector('.masukan-buku-baru').querySelector('h1').remove();
            ...
            console.log("haha" + staticNameMiddle);
        })
    }
}
...
function saveBookToLocalStorage(isSet = false) {
    if (isSet) {
        ...
        return JSON.stringify(new Bookshelf(judul, penulis, tahun, selesai, catatan)); 
    } else {
    ...
    // if (uuid !== undefined) return JSON.stringify(new Bookshelf(judul, penulis, tahun, selesai, catatan, uuid));
}
...
document.getElementById('input-search-text').addEventListener('keyup', () => {
    ...
    for (const data of datas) {
        ...
        console.log("hh = " + a.includes(searchValue));
